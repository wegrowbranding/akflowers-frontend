<?php $title = 'Customer Addresses'; ob_start(); ?>
<div class="d-flex justify-content-between align-items-center mb-3">
    <nav aria-label="breadcrumb"><ol class="breadcrumb mb-0">
        <li class="breadcrumb-item"><a href="<?= url('dashboard') ?>">Dashboard</a></li>
        <li class="breadcrumb-item active">Customer Addresses</li>
    </ol></nav>
    <a href="<?= url('customer-addresses/create') ?>" class="btn btn-primary btn-sm"><i class="bi bi-plus-lg me-1"></i> Add Address</a>
</div>
<div class="card">
    <div class="card-body">
        <form method="GET" action="<?= url('customer-addresses') ?>" class="mb-3 d-flex gap-2" style="max-width:380px">
            <input type="text" name="search" class="form-control form-control-sm" placeholder="Search addresses..." value="<?= e($search) ?>">
            <button class="btn btn-outline-secondary btn-sm px-3"><i class="bi bi-search"></i></button>
            <?php if ($search): ?><a href="<?= url('customer-addresses') ?>" class="btn btn-outline-danger btn-sm"><i class="bi bi-x-lg"></i></a><?php endif; ?>
        </form>
        <div class="table-responsive">
            <table id="addressesTable" class="table table-hover align-middle mb-0">
                ...
                <?php if (empty($addresses)): ?>
                    <tr><td colspan="9" class="text-center text-muted py-4">No addresses found.</td></tr>
                <?php else: foreach ($addresses as $idx => $a): ?>
                    <tr>
                        ...
                        <td><?= e($a['state'] ?? '—') ?></td>
                        <td><?= e($a['pincode'] ?? '—') ?></td>
                        <td><?= $a['is_default'] ? '<span class="badge bg-primary">Default</span>' : '<span class="text-muted">—</span>' ?></td>
                        <td class="text-end">
                            <a href="<?= url('customer-addresses/'.$a['id'].'/edit') ?>" class="btn btn-sm btn-outline-primary me-1"><i class="bi bi-pencil"></i></a>
                            <form method="POST" action="<?= url('customer-addresses/'.$a['id'].'/delete') ?>" class="d-inline" onsubmit="return confirm('Delete this address?')">
...
<?php
$scripts = '<script>$(function(){ $("#addressesTable").DataTable({ paging:false, searching:false, info:false, columnDefs:[{orderable:false,targets:[8]}] }); });</script>';
