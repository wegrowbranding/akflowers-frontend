<?php
$title  = $isEdit ? 'Edit Cart' : 'Add Cart';
$action = $isEdit ? url('carts/'.($cart['id']??'').'/edit') : url('carts/create');
ob_start();
?>
<div class="d-flex justify-content-between align-items-center mb-3">
    <nav aria-label="breadcrumb"><ol class="breadcrumb mb-0">
        <li class="breadcrumb-item"><a href="<?= url('dashboard') ?>">Dashboard</a></li>
        <li class="breadcrumb-item"><a href="<?= url('carts') ?>">Carts</a></li>
    ...
    <div class="card-body">
        ...
        <form method="POST" action="<?= $action ?>">
            <div class="mb-3">
                ...
                <select name="customer_id" class="form-select" required>
                    ...
                    <?php foreach ($customers as $c): ?>
                        <option value="<?= $c['id'] ?>" <?= (string)($cart['customer_id'] ?? '') === (string)$c['id'] ? 'selected' : '' ?>><?= e($c['full_name']) ?> (#<?= $c['id'] ?>)</option>
                    <?php endforeach; ?>
                </select>
            </div>
            <div class="mb-3">
                ...
                <table class="table table-sm table-bordered mb-0">
                    <thead class="table-light"><tr><th>Product</th><th style="width:100px;">Qty</th><th style="width:50px;"></th></tr></thead>
                    <tbody id="itemsBody">
                    <?php 
                    $items = $_SESSION['old']['products'] ?? $cart['items'] ?? [];
                    if (!empty($items)):
                        ...
                        <tr>
                            ...
                            <td><input type="number" name="products[<?= $idx ?>][quantity]" class="form-control form-control-sm" min="1" value="<?= $item['quantity'] ?? 1 ?>" required></td>
            ...
            <div class="d-flex gap-2 mt-3">
                <button type="submit" class="btn btn-primary"><i class="bi bi-check-lg me-1"></i><?= $isEdit ? 'Update' : 'Create' ?> Cart</button>
                <a href="<?= url('carts') ?>" class="btn btn-outline-secondary">Cancel</a>
            </div>
        </form>
    </div>
</div>
...
<tr>
    ...
    <td><input type="number" name="products[{idx}][quantity]" class="form-control form-control-sm" min="1" value="1" required></td>
    <td class="text-center"><button type="button" class="btn btn-sm btn-outline-danger remove-item"><i class="bi bi-trash"></i></button></td>
</tr>
</script>
<script>
$(document).ready(function() {
    let itemIdx = ' . count($_SESSION["old"]["products"] ?? $cart["items"] ?? []) . ';
